...
toolchanger-acme: ## statics for the servoless ACME tool changer (self-lock, engage, preload, hold) -> build/
	$(PY) sim/toolchanger_acme.py

toolchanger-acme-check: ## validate the ACME changer self-locks, reaches 25N, engages <=90 deg, holds
	$(PY) sim/toolchanger_acme.py --check

assembly-insert: ## snap-fit peg-in-socket capture-envelope sweep (the lead-in is the product) -> build/
	$(PY) sim/assembly_insert.py

assembly-insert-check: ## gate: the designed capture radius clears margin*sigma_d (seats by design)
	$(PY) sim/assembly_insert.py --check

harness-demo: ## sim-harness proof: peg self-centers into a V-funnel (renders a gif if headless GL present)
	$(PY) sim/harness_demo.py --demo

harness-demo-check: ## validate the shared sim harness (self-centers + physics gate; needs no GL)
	$(PY) sim/harness_demo.py --selftest
